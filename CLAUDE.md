...
- `src/core/services/` — channel-neutral business logic, called by the bot today and the web API later:
  - `org_service.py` — `OrgService.resolve_context()` maps a channel identity to an internal `UserContext(user_id, org_id)`, **auto-provisioning** user + personal org + membership + channel identity on first contact (keeps single-user behavior working; team onboarding is Fase 1).
  - `expense_service.py` — `ExpenseService`: extract → store receipt → save **draft** (`PENDING_REVIEW`) → `confirm()` (→ `REGISTERED`) / `cancel()`. Owns the string→date parse.
- `src/adapters/`
...
- The interactive prompt is intentionally abstract: action keys (`exp_ok:<id>`) are echoed back verbatim in `IncomingMessage.action`, so confirm/cancel is **stateless** (no in-memory session). A WhatsApp adapter would map the same keys to its own button primitives.
