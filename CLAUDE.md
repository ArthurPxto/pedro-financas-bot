...
- `src/main.py` — composition root only. Builds adapters, injects into services + `BotApplication`, wires the handler into the channel, runs. The `TelegramChannel` is built **before** the services because it doubles as the `Notifier` injected into `BotApplication`. No business logic, no import-time singletons.
