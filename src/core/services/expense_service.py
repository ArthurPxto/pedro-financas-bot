...
from src.core.entities import Expense, ExpenseStatus, NotaDebito
...
from src.core.services.nota_service import first_of_month
...
class ExpenseService:
    ...
    async def confirm(self, ctx: UserContext, expense_id: int) -> Optional[tuple[Expense, NotaDebito]]:
        """Confirma um rascunho e o adiciona como item da nota de débito ABERTA do autor.

        Cria a nota aberta (competência = mês atual) se ainda não houver. Idempotente:
        a inline keyboard persiste após o clique, então só age sobre um rascunho ainda
        PENDING_REVIEW. Devolve (gasto, nota) para o app confirmar em qual nota caiu.
        """
        async with self._uow_factory() as uow:
            ...
            if nota is None:
                nota = await uow.notas.add(
                    NotaDebito(
                        org_id=ctx.org_id,
                        user_id=ctx.user_id,
                        competencia=first_of_month(date.today()),
                    )
                )
            expense.status = ExpenseStatus.CONFIRMED
            expense.nota_id = nota.id
            saved = await uow.expenses.update(expense)
            await uow.commit()
        log.info("gasto confirmado", expense_id=expense_id, nota_id=nota.id)
        return saved, nota
